        ...
        $.ajax({
            ...
            success: function x(greendata) {
                if (200 == greendata.code) {
                    ...
                    var time = '<span class="input-group-addon" id="basic-addon3">' + gettime.getUTCFullYear() + '-' + (gettime.getUTCMonth() + 1) + '-' + gettime.getDate() + '</span>\n' +
                        '                        <input id="user_time" type="time" class="form-control" aria-describedby="basic-addon3">\n';
                    $('#car_message_model').on('show.bs.modal', function (event) {
                        ...
                        modal.find('.modal-body #all_price').html(greens_price_all);
                        modal.find('.modal-body #d_price').html(greens_price_all * 60 / 100);
                    });
...
$('#order_submit').on('click', function () {
    ...
    if (user_token) {
        ...
        var user_time_str = new Date().getUTCFullYear() + '-' + (new Date().getUTCMonth() + 1) + '-' + new Date().getDate() + ' ' + user_time;
        if (user_number >= 1) {
            var info = pd_user_time(user_time);
            if (info == 'Success') {
                $.ajax({
                ...
                $('#login_error').html('<button type="button" class="btn btn-danger pull-left">' + '时间不符合规范' + '</button>')
            } else if (info == 'Warning') {
                $('#login_error').html('<button type="button" class="btn btn-warning pull-left">' + '预定时间离现在不足20分钟,请尽快到店就餐' + '</button>');
                $.ajax({
                    url: window.globalConfig.ROUTE + window.globalConfig.VERSIONS + 'pay/user_pay',
                    dataType: 'json',
                    data: JSON.stringify({
                    ...
                    success: function (pay_data) {
                        if (200 == pay_data.code) {
                            ...
                            console.log(greendata.order_number)
                        } else {
                            $('#login_error').html('<button type="button" class="btn btn-danger pull-left">' + '订单好像存在问题' + '</button>')
                        }
                    }
                });
            } else {
                $('#login_error').html('<button type="button" class="btn btn-warning pull-left">' + '订单好像存在问题' + '</button>')
            }
        } else {
...
function pd_user_time(user_time) {
    var os_time = new Date().getTime();
    var user_time_str = new Date().getUTCFullYear() + '-' + (new Date().getUTCMonth() + 1) + '-' + new Date().getDate() + ' ' + user_time;
    var user_times = Date.parse(user_time_str);
    if (user_times - os_time <= 0) {
        return 'Danger'
    } else if (user_times - (os_time + 20 * 1000 * 60) <= 0) {
        return 'Warning'
    } else {
        return 'Success'
    }
